...
//parametros a serem definidos externamente.
...
static int tmp_symbol_index = 0;0x
...
static _Bool f_wait_for_ack = 0;1
...
p_final: /// Check the Reed-solomon, print the DATA frame, and send the ACK
    ...
    if (payload_len>0) { // Data frame
        if (mac_or_app == APP) { // To upper layer
            //rx_pkt = kmalloc (sizeof (struct vlc_vlc_devspacket), GFP_KERNEL);
